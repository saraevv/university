    ...
    case WM_PAINT:
        {
			...
			POINT l = { rect.left,rect.top }, r = { rect.right,rect.bottom };
			ClientToScreen(hWnd, &l);
			ClientToScreen(hWnd, &r);
			n=sprintf_s(res, "%d %c %d %c %d %c %d", l.x, ';', l.y,';' ,r.x, ';', r.y);
			MultiByteToWideChar(CP_UTF8,
				MB_ERR_INVALID_CHARS, res, strlen(res), wcharr, 100);
			DrawText(hdc, wcharr, n, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
            EndPaint(hWnd, &ps);
        }
        break;
	case WM_MOVE:
